...
class Solution(object):
    ...
    def _to_calculate(self,sign,_num,_sum):
        if sign == "+":
            if _num !="": _sum.append(int(_num))
        elif sign == "-":
            if _num !="": _sum.append(-int(_num)) 
        elif sign == "*":
            _last = _sum.pop()
            if _num !="": _sum.append(_last * int(_num)) 
        elif sign == "/":
            _last = _sum.pop()
            _val = (_last // int(_num)) if _last >= 0 else -(abs(_last) // int(_num))
            # print "## / ",_last,_num, _val
            _sum.append(_val) if int(_num) != 0 else _sum.append(0)
        return _sum
    ...
    def _help(self,s):
        ...
        while s:
            ...
            if current_str == " ": 
                s.pop(0)
                continue
            elif '0' <= current_str <= '9':
                _num = _num + current_str
                s.pop(0)
            elif current_str in ['+','-','*','/',")"]:
                _sum = self._to_calculate(sign,_num,_sum)
