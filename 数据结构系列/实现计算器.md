...
[tonystevenj](https://github.com/tonystevenj) 提供 Java 解法代码：
```java
class Solution {
    public int calculate(String s) {
        char[] sc = s.replaceAll(" ","").toCharArray();
        int[] index = new int[]{0};
        return core(sc,index);
    }
    int core(char[] sc, int[] index){
        Deque<Integer> stc =  new ArrayDeque<>();
        while(index[0]<sc.length){
            if(sc[index[0]]==')'){
                index[0]++;
                break;
            }
            char sign = '+';
            if(!Character.isDigit(sc[index[0]])&&sc[index[0]]!='('){
                sign = sc[index[0]];
                index[0]++;
            }
            int number =0;
            if(sc[index[0]]=='('){
                index[0]++;
                number = core(sc,index);
            }else{
                while (index[0]<sc.length&&Character.isDigit(sc[index[0]])) {
                    number = number * 10 + (int)(sc[index[0]] - '0');
                    index[0]++;                    
                }
            }
            if(sign=='-'){
                ...
                System.out.println("sign not correct");
            }
        }
