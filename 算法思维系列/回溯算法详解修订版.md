...
[LingfengZhu](https://github.com/LingfengZhu) 提供**N皇后问题** Python3 解法代码：

```python
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
    # 输入棋盘边长 n，返回所有合法的放置
        def backtrack(first=0, path=[]):
            '''
            回溯算法：
            路径：当前行之前的每一行都已经成功放置了皇后
            选择列表：当前行的所有列都是放置皇后的选择
            结束条件：到达 board 的最后一行
            '''
            if first == n: # 触发结束条件：已经到达 board 的最后一行
                output.append(['.' * i + 'Q' + '.' * (n-i-1) for i in path[:]])
            for i in range(n): # 所有可选项：当前行的所有列
                if col_valid[i] and uphill_valid[first+i] and downhill_valid[first-i]: # 仅考虑所有合法的可选项
                    # 进行选择
                    path.append(i) # 当前位置加入路径
                    col_valid[i] = uphill_valid[first+i] = downhill_valid[first-i] = False # 更新冲突情况
                    ...
                    # 回溯：撤销选择
                    path.pop(-1) # 当前位置退出路径
                    col_valid[i] = uphill_valid[first+i] = downhill_valid[first-i] = True # 更新冲突情况
            
        # 定义三个字典用于判断各位置是否有皇后冲突: 若无冲突则对应值为 True
        col_valid = dict() # 用于判断列上是否有皇后冲突
        uphill_valid = dict() # 用于判断左下到右上的对角线上是否有皇后冲突
        downhill_valid = dict() # 用于判断左上到右下的对角线上是否有皇后冲突
        
        # 初始化 col_valid
        for i in range(n):
            col_valid[i] = True
        
        # 初始化 uphill_valid 与 downhill_valid
        for i in range(n):
            for j in range(n):
                uphill_valid[i + j] = True # 位于同一条左下到右上对角线的位置满足：横纵坐标之和为定值
                downhill_valid[i - j] = True # 位于同一条左上到右下对角线的位置满足：横纵坐标之差为定值
