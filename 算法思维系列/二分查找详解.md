...
def search(self, nums: List[int], target: int) -> int:
        left = 0 #左边点起始位置（index = 0）
        right = len(nums) - 1 #最后一个位置（index = 长度-1）
        while left <= right:
            #循环的判定条件如果左边的index大于等于右边的index就退出循环
            #每一次循环都找left和right中间的index去和target数字对比
            mid = left + (right-left)//2  #mid是数组中间的index
            if nums[mid] == target:
                #如果对应的index就是target，直接返回index就是mid
                return mid 
            elif nums[mid]<target:
                #如果对应的index的数字小于target，left直接等于mid+1的index，相当于数组直接左边的一半不再看了
                left = mid+1
            else:
                #同理，right = 中间index-1，直接抛弃右边的一半
                right = mid-1
