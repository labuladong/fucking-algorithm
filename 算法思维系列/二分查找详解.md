...
[Ellen Tan](https://github.com/ellenxtan) 提供Find Minimum in Rotated Sorted Array I Python3解法代码： 
```python
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while l+1<r:
            m = l + (r-l)//2
            if nums[m] <= nums[r]:  # 判断中指针与右指针大小
                # 中指针在后一半的array
                r = m
            else:  # 中指针在前一半的array
                l = m
        return min(nums[l], nums[r])  # 返回左指针和右指针中的最小值
```

[Ellen Tan](https://github.com/ellenxtan) 提供Find Minimum in Rotated Sorted Array II Python3解法代码：  
```python
class Solution:
    def findMin(self, nums: List[int]) -> int:
        l = 0  # 左指针
        ...
        while l+1<r:
            m = l + (r-l)//2
            if nums[l]==nums[m]==nums[r]:  # 可以减小右指针，也可以增加左指针
                r -= 1  # 或者 l+=1
            elif nums[m]<=nums[r]:  # 最小值在后一半的array
                r = m
            else:
                l = m
        return min(nums[l], nums[r])  # 返回左指针和右指针中的最小值
...
[Ellen Tan](https://github.com/ellenxtan) 提供Find Peak Element Python3解法代码：  
```python
class Solution:
    def findPeakElement(self, nums: List[int]) -> int:
        nums = [float('-inf')] + nums + [float('-inf')]  # 保证nums至少有三个元素
        l = 0
        r = len(nums)-1
        while l+1<r:
            m = l + (r-l)//2
            if nums[m-1]<nums[m] and nums[m]>nums[m+1]:  # 为峰值
                return m-1  # 因为第0为为-inf，返回的索引要减一
            elif nums[m-1]<=nums[m]<=nums[m+1]:  # 数列递增
                l = m
            elif nums[m-1]>=nums[m]>=nums[m+1]:  # 数列递减
                r = m
            else:  # 为底，增加左指针或者减小右指针均可
                r = m  # 或 l = m
        # check左指针和右指针是否满足条件
