...
```
**二分查找的扩充**：
以这个思路，「寻找左侧边界的二分搜索」寻找的其实是从左往右第一个满足if内条件的数， 「寻找右侧边界的二分查找」其实是从左往右找第一个不满足条件的数字
比如[540. 有序数组中的单一元素](https://leetcode-cn.com/problems/single-element-in-a-sorted-array/)
```
class Solution {
    public int singleNonDuplicate(int[] nums) {
        ...
        while (left < right) {
            int mid = left + (right - left) / 2;
            if (nums[2 * mid - 2] == nums[2 * mid - 1]) {
                left = mid + 1;
            } else {
                right = mid;
            }
        }
        return nums[2 * left - 2];
    }
}
```
