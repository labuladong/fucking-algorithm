...
[huangjuncong](https://www.zhihu.com/people/huang-jie-89-33) 提供java解法代码：

```java
    public int[][] merge(int[][] intervals) {
        // 以interval[i][0]为key进行排序
        Arrays.sort(intervals, (v1, v2) -> v1[0] - v2[0]);
        int[][] res = new int[intervals.length][2];
        int idx = 0;
        for(int i = 0;i<intervals.length;i++){
            //无法合并（新区间的start>旧区间的end）
            ...
            //可以合并，更新end
                ...
                res[idx - 1][1] = Math.max(intervals[i][1], res[idx - 1][1]);
            }
        }
