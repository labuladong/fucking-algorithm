...
[labuladong](https://github.com/labuladong) 提供 Java 解法代码：

```java
public int maxProfit(int[] prices) {
    // 如果是第0天，还未开盘，此时有如下初始状态
    int dp_i_0 = 0;
    int dp_i_1 = Integer.MIN_VALUE; // 第0天不可能持有股票
    // 状态转移
    for (int price : prices) {
...
[sakuragi1111](https://github.com/sakuragi1111) 提供 Python3 解法代码：

```python
def maxProfit(self, prices: List[int]) -> int:
    # 如果是第0天，还未开盘，此时有如下初始状态
    dp_i_0 = 0
    dp_i_1 = float('-inf') # 第0天不可能持有股票
    # 状态转移
    for price in prices:
...
public int maxProfit(int[] prices) {
    ...
    // 对第0天进行初始化，由于不可能持有股票，以负无穷大标记
    // dp[i][1]在dp[i][1] + price中被使用，由于股价非负，所以不会溢出
    for (int i = 0; i <= k; i++) {
...
**4.【[买卖股票的最佳时机IV](https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-iv/)】**
