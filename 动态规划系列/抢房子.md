...
[daiyi-guizhou](https://github.com/daiyi-guizhou/) 提供 House Robber I python 解法代码：
```py
class Solution(object):
    def rob(self, nums):
        ...
        for j in range(n-1,-1,-1):
            dp_i = max(dp_i_1, nums[j] + dp_i_2)
            dp_i_2 = dp_i_1
            dp_i_1 = dp_i
        return dp_i
```

[daiyi-guizhou](https://github.com/daiyi-guizhou/) 提供 House Robber II python 解法代码：
...
[daiyi-guizhou](https://github.com/daiyi-guizhou/) 提供 House Robber III python 解法代码：
...
class Solution(object):
    ...
    def sub_rob(self, node):
        if not node: return 0
        if node in self.memo: return self.memo[node]
        do_it = node.val 
        if node.left: do_it = do_it + self.sub_rob(node.left.left) + self.sub_rob(node.left.right)
        if node.right: do_it = do_it + self.sub_rob(node.right.left) + self.sub_rob(node.right.right)
        not_do = self.sub_rob(node.left) + self.sub_rob(node.right)
...
class Solution(object):
    def rob(self, root):
        ...
        :rtype: int
        """
        res = self.sub_rob(root)
        return max(res)

    def sub_rob(self, node):
        if not node: return [0, 0]
        left = self.sub_rob(node.left)
        right = self.sub_rob(node.right)
        do_it = node.val + left[0] + right[0]
