...
class Solution:
    ...
    # 计算 p[j..] 是否匹配 s[i..]
    def dp(self, s: str, i: int, p: str, j: int, memo) -> bool:
        # 记忆化搜索: 记录状态 (i, j)，消除重叠子问题
        if (i, j) in memo:
            return memo[(i, j)]

        res = False
        # 当pattern用完了，source也必须用完才算完全匹配
        if j == len(p):
            res = i == len(s)

        else:
            # source首字母和pattern首字母匹配情况
            first_match = i < len(s) and (p[j] == s[i] or p[j] == '.')

            if j + 1 < len(p) and p[j + 1] == '*':
                res = self.dp(s, i, p, j + 2, memo) or \
                    (first_match and self.dp(s, i + 1, p, j, memo))
            else:
                res = first_match and \
                    self.dp(s, i + 1, p, j + 1, memo)
