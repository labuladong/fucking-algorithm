...
[dayao-live](https://github.com/dayao-live) 提供 Java 代码：

```java
//带备忘录的递归, Recursion with Memoization
public static boolean isMatch(String s, String p) {
    int[][] memo = new int[s.length()+1][p.length()+1];
      for(int m[] : memo){
          Arrays.fill(m,-1); //-1 represent unvisited
      }
    return dp(s,p,memo,0,0);		
}   	

  private static boolean dp(String s,String p, int[][] memo, int i, int j){
    ...
    if(j<=p.length()-2&&p.charAt(j+1)=='*') {
      ans=dp(s,p,memo,i,j+2)||(first&&dp(s,p,memo,i+1,j));
    }
    else {
      ans=first&&dp(s,p,memo,i+1,j+1);
    }
    if(ans) memo[i][j]=1;
    if(!ans) memo[i][j]=0;
    return ans;
