...
[labuladong](https://github.com/labuladong) 提供python的解法代码：
```python
...
public:
    int superEggDrop(int K,int N) {
        int dp[K + 1][N + 1];     // dp[i][j] : i = [0,K] 个鸡蛋，j = [0,N] 次扔鸡蛋，能测试的楼层数        
        memset(dp,0,sizeof dp);   // 全部初始化为 0

        int m = 1;                // 第一次测试
        while(dp[K][m] < N) {               // 外层循环：能确定的楼层数小于 N
            for(int k = 1 ; k <= K ; ++k)   // 内层循环：穷举鸡蛋的个数 
                dp[k][m] = dp[k][m - 1] + dp[k - 1][m - 1] + 1; // (本次鸡蛋不碎，剩下 m - 1 次扔鸡蛋机会)能确定的楼层数 + 
                                                                // (本次鸡蛋碎，剩下 m - 1 次扔鸡蛋机会)能确定的楼层数 +
                                                                // 本楼层得到确定
            
            if(dp[K][m] >= N)   return m;                       // 当能确定的楼层数大于等于 N 时，返回扔鸡蛋的次数。
