...
对于一个问题，你可能都很难想到它跟二分查找有关，比如前文 [最长递增子序列](https://labuladong.online/algo/fname.html?fname=动态规划设计：最长递增子序列) 就借助一个纸牌游戏衍生出二分查找解法。
...
在前文 [二分查找详解](https://labuladong.online/algo/fname.html?fname=二分查找详解) 中，详解了如何正确写出三种二分查找算法的细节。二分查找返回目标值 `val` 的索引，对于搜索**左侧边界**的二分查找，有一个特殊性质：
...
以上就是搜索左侧边界的二分查找，等会儿会用到，其中的细节可以参见前文 [二分查找详解](https://labuladong.online/algo/fname.html?fname=二分查找详解)，这里不再赘述。
