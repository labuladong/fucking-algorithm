...
[乾巧](https://www.zhihu.com/people/wu-suo-wei-wu-suo-wei-78) 提供 golang 代码：

```golang
func countPrimes(n int) int {
	count := 0
	signs := make([]bool, n+1)  // bool 默认为 false
	for i := 2; i < n; i++ { 
		if signs[i] { // 如果为 true，说明不是素数
			continue
		}
        count++
		for j := i * i; j < n; j += i {
			signs[j] = true  // 将 i 的倍数置为 true
		}
	}
	return count
}
