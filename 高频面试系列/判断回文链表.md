...
[yussuy](https://github.com/yussuy) 提供 Java 代码：
```java
/**
...
class Solution {
    public boolean isPalindrome(ListNode head) {
        ...
        // 如果在这里直接用head会改变head的值，但传入函数中作为参数值传递就不会改变。后续还能再使用head。
        cut(head, slow);
        return isEqual(head, reverse(slow));
    }

    private void cut(ListNode head, ListNode l) {
        while (head.next != l) {
            head = head.next;
        }
        head.next = null;
    ...
    private boolean isEqual(ListNode l1, ListNode l2) {
        while (l1 != null) { // l1 有可能和 l2 一样长，有可能比 l2 长1
            if (l1.val != l2.val) 
                return false;
            l1 = l1.next;
            l2 = l2.next;
