...
[solejay](https://github.com/purenjie) 提供 Python3 代码：

```python
def isValid(str):
    left = []
    for c in str:
        if c == '(' or c == '{' or c == '[': # 字符 c 是左括号
            left.append(c)
        else: # 字符 c 是右括号
            if len(left) != 0 and leftOf(c) == left[-1]: # 左括号不空且左右括号匹配
                left.pop() 
            else: # 和最近的左括号不匹配
                return False
...
def isValid(str):
    ...
    dict = {"]":"[", "}":"{", ")":"("} # 用字典实现左右括号的键值对
    for c in str:
        if c in dict.values(): # 左括号加入
            left.append(c)
        elif c in dict.keys(): # 右括号判断
            if left == [] or dict[c] != left.pop(): # pop 之后列表减少一个元素
                return False
        else: # 其他符号或字符
            return False
    return len(left) == 0
